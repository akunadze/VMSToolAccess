...
void MFRC522::PCD_WriteRegister(PCD_Register reg, byte value) {
	...
	gpio_set_level(_chipSelectPin, false); // Select slave
	m_spi.transfer(data, 2);
	gpio_set_level(_chipSelectPin, true); // Release slave
...
void MFRC522::PCD_Init() {
	...
	ESP_LOGD(LOG_TAG, "CS = %d, RS = %d", _chipSelectPin, _resetPowerDownPin);

	gpio_set_direction(_chipSelectPin, GPIO_MODE_OUTPUT);
	m_spi.init();
	...
	if (_resetPowerDownPin != UNUSED_PIN) {
		...
		if (!gpio_get_level(_resetPowerDownPin)) {	// The MFRC522 chip is in power down mode.
			gpio_set_direction(_resetPowerDownPin, GPIO_MODE_OUTPUT);
			gpio_set_level(_resetPowerDownPin, true); // Exit power down mode. This triggers a hard reset.
...
void MFRC522::PCD_Init(byte chipSelectPin, byte resetPowerDownPin) {
	_chipSelectPin = (gpio_num_t)chipSelectPin;
	_resetPowerDownPin = (gpio_num_t)resetPowerDownPin;
	// Set the chipSelectPin as digital output, do not select the slave yet
	PCD_Init();
} // End PCD_Init()
